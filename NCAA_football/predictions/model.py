"""
Bayesian conjugate linear-regression model for NCAAF point spreads.
...
Sequential ("online") update — the current posterior is used as the prior for
the next batch of games, so the model literally improves week over week:
    Lambda' = Lambda + X^T X
    eta'    = eta    + X^T y
    a'      = a + n/2
...
Between seasons (and optionally each week) we apply a forgetting/discount
factor that widens predictive uncertainty and lets recent data carry more
weight, without throwing away what was learned. Point estimates are preserved;
only the *confidence* (effective sample size) shrinks.
...
class BayesianSpreadModel:
    def __init__(self,
                 prior_coef_sd=10.0,      # prior sd of each (standardized) slope, in points
                 prior_intercept_sd=60.0,  # weak prior on the constant term
                 prior_sigma=16.0,         # prior residual sd of a game spread (points)
    ...
    def discount(self, factor):
        ...
        Applied at season boundaries (and optionally weekly)."""
        f = float(factor)
        if not (0 < f <= 1):
            return
    ...
    def predict(self, X):
        ...
        pred_spread  = posterior-predictive mean of (home_points - away_points)
        win_prob_home = P(spread > 0)
        """
        Xs = self._transform(X)
        m = self.mean_beta
        nu = 2.0 * self.a
